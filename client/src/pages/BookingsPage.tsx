...
import { getMyBookingsApi, cancelBookingApi, completeBookingApi } from '../api/bookings';
...
const isPastBooking = (b: Booking) => {
  const iso = b.bookedDate.includes('T') ? b.bookedDate : `${b.bookedDate}T${b.bookedTime || '23:59'}`;
...
export default function BookingsPage() {
  ...
  const handleSubmitReview = async (bookingId: string) => {
    ...
    setReviewError('');
    try {
      await createReviewApi({ bookingId, rating, comment: comment.trim() || undefined });
      setReviewedIds((prev) => [...prev, bookingId]);
      setReviewFor(null);
    } catch (err: unknown) {
      setReviewError(err instanceof ApiError ? err.message : 'Грешка при изпращане на отзива');
    } finally {
  ...
  return (
    <Layout>
      <div className={styles['bookings__header']} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' }}>
        <div>
        ...
        <button onClick={() => setChoiceOpen(true)} className={styles['bookings__new-btn']}>
      ...
      <InfoBanner
        ...
        steps={['Запази час (със или без AI анализ)', 'Посети сервиза', 'Завърши и оцени обслужването']}
      />
              ...
              <div key={b._id} className={styles['bookings__card']}>
                <div className={styles['bookings__card-inner']}>
                  ...
                  <div className={styles['bookings__actions']}>
                    {(b.status === 'pending' || b.status === 'confirmed') && (
                      <>
                        <button
                          ...
                          title={!isPastBooking(b) ? 'Можеш да отбележиш като завършена след часа на резервацията' : undefined}
                    ...
                    {b.status === 'completed' && reviewedIds.includes(b._id) && (
                      <span className={styles['bookings__reviewed']}>Оценено ✓</span>
                    )}
                  </div>
                </div>
                ...
                {reviewFor === b._id && (
                  <div className={styles['bookings__review-form']}>
                    <p className={styles['bookings__review-title']}>Оцени обслужването</p>
                    ...
                    />
                    {reviewError && <p className={styles['bookings__review-error']}>{reviewError}</p>}
